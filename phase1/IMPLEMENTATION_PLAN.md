...
| 1 | テスト用PDF複数枚で動作確認 | [x] |
| 2 | 回答品質の手動評価（5件以上） | [x] |
| 3 | プロンプト改善（`PROMPT_VERSION`を更新し比較評価） | [x] |
| 4 | トークン数・概算コストの表示 | [ ] |
| 5 | エラーケースの潰し込み | [x] |
| 6 | レスポンス速度の確認と改善 | [x] |
| 7 | PDFサイズ上限・ページ数上限・非対応形式（画像PDF等）の検証テスト | [x] |
| 8 | APIタイムアウト・リトライ・レート制限の負荷/異常系テスト | [x] |
| 9 | ログ保存ポリシー（PIIマスキング・保持期間）の運用検証 | [ ] |
| 10 | エラー時ユーザー表示文言ポリシーのUI/UXレビューと改善 | [x] |
| 11 | 評価ログ保存先の運用（`phase1/docs/qa_evaluation_log.md`） | [x] |


### 評価ログ運用手順（Week 9〜10）

- **評価テンプレート**: `phase1/docs/qa_evaluation_template.md`
- **評価ログ保存先**: `phase1/docs/qa_evaluation_log.md`

1. プロンプト改善前に `PROMPT_VERSION` と評価対象モデルを確定する。
2. C01〜C05の同一質問セットで評価を実施し、評価ログへ追記する。
3. プロンプト改善時は `PROMPT_VERSION` を更新して再評価し、同一PDF・同一質問で比較する。
4. Fail/Borderlineの行には改善メモを必ず残し、次スプリントの実装タスクへ転記する。


### Week 9〜10 完了チェック更新（2026-05-08）

- 完了更新: #1 #2 #3 #5 #6 #7 #8 #10 #11。
- **未完了（次週へ再計画）**:
  1. #4 トークン数・概算コストの表示（`src/lib/pricing.ts` とUI表示の実装が未着手）
  2. #9 ログ保存ポリシー（PIIマスキング・保持期間）の運用検証（実運用ログでの監査手順が未整備）

#### 次週タスク（Week 11先行着手）

1. `usage` 由来のトークン表示とモデル別概算コスト計算をAPIレスポンスへ追加。
2. チャットUIに「今回のトークン/概算コスト」を表示し、モデル未定義時のフォールバックを実装。
3. ログ監査チェックリスト（PIIマスキング確認・保持期間自動削除確認）を `phase1/docs` に追加。
4. 429/Timeout系のエラー文言改善をUIへ反映し、再評価ログへ追記。
