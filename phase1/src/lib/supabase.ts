...
const classifyStorageError = (error: unknown): StorageSaveErrorType => {
  const details = String((error as { message?: string; code?: string; statusCode?: string })?.message ?? '').toLowerCase()
  const code = String((error as { code?: string; statusCode?: string })?.code ?? (error as { statusCode?: string })?.statusCode ?? '').toLowerCase()
...
export const savePdfToStorage = async (sessionId: string, fileName: string, binary: ArrayBuffer | Buffer, contentType: string) => {
...
export const deletePdfFromStorage = async (storagePath: string) => {
  const supabase = createSupabaseServiceClient()
